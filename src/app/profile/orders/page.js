          ...
          {orders.map(order => (
            <div key={order.id} className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-1">{order.gigTitle}</h3>
                <p className="text-gray-500 text-sm">Package: <span className="font-semibold text-gray-700">{order.packageName}</span></p>
              </div>
              ...
              <Link href={`/gig/${order.gigId}/view`} className="md:ml-4 bg-gray-50 hover:bg-gray-100 text-gray-700 p-3 rounded-xl border border-gray-200 transition-colors flex items-center justify-center" title="View Gig">
